...
entity PL_CLASSIFIER_w_VOTING is
  ...
  bram_wrdata_Pre_Comp_Vect : in std_logic_vector( PCV_DATA_WIDTH - 1 downto 0 );
  bram_en_Pre_Comp_Vect     : in std_logic;
  bram_we_Pre_Comp_Vect     : in std_logic;
  -- RAM Kernel Scale
  bram_addr_Kernel_Scale    : in std_logic_vector( ADDR_WIDTH -1 downto 0 ); 
  bram_wrdata_Kernel_Scale  : in std_logic_vector( KERNEL_DATA_WIDTH-1 downto 0 ); 
  bram_en_Kernel_Scale      : in std_logic;
  bram_we_Kernel_Scale      : in std_logic; 
  -- RAM Bias
  bram_addr_Bias            : in std_logic_vector( ADDR_WIDTH -1 downto 0 ); 
  bram_wrdata_Bias          : in std_logic_vector( BIAS_DATA_WIDTH-1 downto 0 ); 
...
signal i_out_data_sipo   : std_logic_vector( (N_ATTRIBUTE*ATTRIBUTE_DATA_WIDTH)-1 downto 0); -- 8bit di attributo *34, in 2's complement  
-------------------------------
-- Classifier signal 
-------------------------------
type t_Attr_j is array (0 to ATTRIBUTE_DATA_WIDTH - 1 ) of std_logic_vector( ATTRIBUTE_DATA_WIDTH-1 downto 0 );
signal i_Attr_classifier : Attr_in_array_of_std_logic_vector;--t_Attr_j;--
type t_Pre_comp_vect is array (0 to ATTRIBUTE_DATA_WIDTH -1) of std_logic_vector( PCV_DATA_WIDTH-1 downto 0 );
signal i_Pre_comp_vect   : Acc_SVxAlphaxSVLabel_in_array_of_std_logic_vector;--t_Pre_comp_vect;--
signal i_Kernel_Scale    : std_logic_vector( KERNEL_DATA_WIDTH -1 downto 0 );
signal i_Bias            : std_logic_vector( BIAS_DATA_WIDTH -1 downto 0 );
...
-------------------------------
signal i_addr_RAM_Pre_Computed_Vector :  std_logic_vector(ADDR_WIDTH   -1 downto 0);  -- Address bus, width determined from RAM_DEPTH
signal i_din_RAM_Pre_Computed_Vector  :  std_logic_vector(PCV_DATA_WIDTH -1 downto 0);  -- RAM input data
...
signal i_dout_RAM_Pre_Computed_Vector :  std_logic_vector(PCV_DATA_WIDTH -1 downto 0);  -- RAM output data 
-------------------------------
-- RAM_Kernel_Scale signal 
-------------------------------
signal i_addr_RAM_Kernel_Scale       : std_logic_vector(ADDR_WIDTH  -1 downto 0);     -- Address bus, width determined from RAM_DEPTH
signal i_din_RAM_Kernel_Scale        : std_logic_vector(KERNEL_DATA_WIDTH -1 downto 0);     -- RAM input data
...
signal i_regce_RAM_Kernel_Scale      : std_logic := '1';                     -- Output register enable
signal i_dout_RAM_Kernel_Scale       : std_logic_vector (KERNEL_DATA_WIDTH -1  downto 0);
-------------------------------
-- RAM_Bias signal 
-------------------------------
signal i_addr_RAM_Bias :  std_logic_vector( ADDR_WIDTH -1 downto 0);                  -- Address bus, width determined from RAM_DEPTH
signal i_din_RAM_Bias  :  std_logic_vector( BIAS_DATA_WIDTH -1 downto 0);                  -- RAM input data
...
signal i_regce_RAM_Bias:  std_logic := '1';                                  -- Output register enable
signal i_dout_RAM_Bias :  std_logic_vector( BIAS_DATA_WIDTH- 1 downto 0);
-------------------------------
-- Voting_w_ce signal 
-------------------------------
...
signal i_FSM_addr_RAM_Pre_Computed_Vector   : std_logic_vector( ADDR_WIDTH -1 downto 0 );
signal i_FSM_en_RAM_Pre_Computed_Vector     : std_logic;
signal i_FSM_addr_RAM_Kernel_Scale          : std_logic_vector( ADDR_WIDTH -1 downto 0 );
...
component Classifier is
  ...
  Kernel_Scale_inv_quadro_SVM : in  std_logic_vector( KERNEL_DATA_WIDTH-1 downto 0 ); -- Kernel_Scale^-2 
  Bias_SVM                    : in  std_logic_vector( BIAS_DATA_WIDTH-1 downto 0 ); -- Bias da aggiungere alla fine 
...
component SIPO_buffer is
    ...
    out_data : out std_logic_vector( N_ATTRIBUTE*ATTRIBUTE_DATA_WIDTH-1 downto 0)-- 8bit di attributo in 2's complement *34 
    );
end component;

component RAM_Pre_Computed_Vector is
  Port (
  addr_RAM_Pre_Computed_Vector : in std_logic_vector( ADDR_WIDTH-1 downto 0);    -- Address bus, width determined from RAM_DEPTH
  din_RAM_Pre_Computed_Vector  : in std_logic_vector(PCV_DATA_WIDTH-1 downto 0);                -- RAM input data
  ...
  dout_RAM_Pre_Computed_Vector : out std_logic_vector(PCV_DATA_WIDTH-1 downto 0)               -- RAM output data 
);
end component;

component RAM_Kernel_Scale is
  Port (
  addr_RAM_Kernel_Scale : in std_logic_vector(ADDR_WIDTH -1 downto 0);    -- Address bus, width determined from RAM_DEPTH
  din_RAM_Kernel_Scale  : in std_logic_vector(KERNEL_DATA_WIDTH-1 downto 0);                -- RAM input data
  ...
  dout_RAM_Kernel_Scale : out std_logic_vector(KERNEL_DATA_WIDTH-1 downto 0)               -- RAM output data 
);
end component;

component RAM_Bias is
  Port (
  addr_RAM_Bias : in std_logic_vector(ADDR_WIDTH -1 downto 0);    -- Address bus, width determined from RAM_DEPTH
  din_RAM_Bias  : in std_logic_vector(BIAS_DATA_WIDTH -1 downto 0);                -- RAM input data
  ...
  dout_RAM_Bias : out std_logic_vector(BIAS_DATA_WIDTH -1 downto 0)               -- RAM output data 
);
end component;
...
component FSM is 
  ...
  addr_RAM_Pre_Computed_Vector   : out std_logic_vector( ADDR_WIDTH - 1 downto 0 );                    
  en_RAM_Pre_Computed_Vector     : out std_logic;                                           
  -- RAM Kernel Scale                                                                       
  addr_RAM_Kernel_Scale          : out std_logic_vector( ADDR_WIDTH - 1 downto 0 );                    
  en_RAM_RAM_Kernel_Scale        : out std_logic;                                           
  -- RAMs Bias                                                                              
  addr_RAM_Bias                  : out std_logic_vector( ADDR_WIDTH -  1 downto 0 );                    
...
for_in_classifier:
for i in  0 to N_ATTRIBUTE -1 generate 
   i_Attr_classifier(i)(ATTRIBUTE_DATA_WIDTH-1 downto 0) <= i_out_data_sipo(271 - ATTRIBUTE_DATA_WIDTH*i downto 264 - ATTRIBUTE_DATA_WIDTH*i );
   i_Pre_comp_vect(i)( 12-1 downto 0) <= i_dout_RAM_Pre_Computed_Vector(PCV_DATA_WIDTH-1 - i*12 downto 396 - 12*i);
end generate;
...
m_axis_tdata( AXIS_DATA_WIDTH -1 downto 3 ) <= ( others => '0' );


                                    -- input da PS con i_BRAM_PS_to_PL = '1'                                   -- input da FSM
i_addr_RAM_Pre_Computed_Vector  <= bram_addr_Pre_Comp_Vect( ADDR_WIDTH -1 downto 0 )         when (i_BRAM_PS_to_PL = '0') else i_FSM_addr_RAM_Pre_Computed_Vector;   
i_din_RAM_Pre_Computed_Vector   <= bram_wrdata_Pre_Comp_Vect( PCV_DATA_WIDTH-1 downto 0 )    when (i_BRAM_PS_to_PL = '0') else (others => '0');                      
...
i_addr_RAM_Kernel_Scale         <= bram_addr_Kernel_Scale( ADDR_WIDTH-1 downto 0 )           when (i_BRAM_PS_to_PL = '0') else i_FSM_addr_RAM_Kernel_Scale;          
i_din_RAM_Kernel_Scale          <= bram_wrdata_Kernel_Scale( KERNEL_DATA_WIDTH-1 downto 0 )  when (i_BRAM_PS_to_PL = '0') else (others => '0');                      
...
i_addr_RAM_Bias                 <= bram_addr_Bias( ADDR_WIDTH-1 downto 0 )                   when (i_BRAM_PS_to_PL = '0') else i_FSM_addr_RAM_Bias;                  
