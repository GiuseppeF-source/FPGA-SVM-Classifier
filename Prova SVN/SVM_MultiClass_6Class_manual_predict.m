...
%        apertura file in scrittura e assegnazione ID
       ...
       f_Bias_id = fopen(percorso_partizione+"Bias_ddr32.h",'wt');
       f_Kernel_Scale_id = fopen(percorso_partizione+"Kernel_Scale_ddr32.h",'wt');
       f_Acc_SVxAlphaxSVLabel_id = fopen(percorso_partizione+"Acc_SVxAlphaxSVLabel_ddr32.h",'wt');
       f_Attribute_Test_id = fopen(percorso_partizione+"Attribute_Test_ddr32.h","wt");
...
%             % Salvataggio su File 
            if (indx_Attribute_Test == 1) % per farlo una sola volta
                fprintf(f_Bias_id,"0b0000000000000000000000000%s,\n", bin( Bias_fi )  );
                
                fprintf(f_Kernel_Scale_id,"0b00000000000000000000%s,\n", bin( Kernel_Scale_inv_quadr_fi  ) );
                ...
                for s = 0:1:33
                    ...
                    inizio = s*15+1;
                    fine = s*15+12;
                    fprintf(f_Acc_SVxAlphaxSVLabel_id,"0b00000000000000000000%s,\n",string_vect_pre_computed(1,inizio:fine) );
                end
...
%               % formare unica word da 8bit * 34 Attributi
            ...
            string_Attribute_Test = bin (...
                ...
                for j = 0:1:33
                    ...
                    fine = j*11+8;
                    if (j == 4-1 || j == 8-1 || j == 12-1 || j == 16-1 || j == 20-1 || j == 24-1 || j == 28-1 || j == 32-1 )  
                      fprintf(f_Attribute_Test_id,"%s,\n",string_Attribute_Test(1,inizio:fine) );
                    elseif (j == 0 || j == 4 || j == 8 || j == 12 || j == 16 || j == 20 || j == 24 || j == 28 || j == 32)
                      fprintf(f_Attribute_Test_id,"0b%s",string_Attribute_Test(1,inizio:fine) );  
                    else
                      fprintf(f_Attribute_Test_id,"%s",string_Attribute_Test(1,inizio:fine) );
                    end
                end
                fprintf(f_Attribute_Test_id,"0000000000000000,\n");
            end
